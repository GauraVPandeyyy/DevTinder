...
import { Loader2, Mail, Lock, Eye, EyeOff, ComputerIcon } from "lucide-react";
...
const Login = () => {
  ...
  const loginHandler = async (e) => {
    ...
    try {
      ...
      // Handling both generic message and the array error format we discussed earlier
      ...
      >
        ...
        <div className="bg-white/[0.02] border border-white/5 p-6 md:p-8 rounded-[2rem] shadow-2xl backdrop-blur-xl">
          <form onSubmit={loginHandler} className="space-y-5">
            ...
            >
              {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : "Access Terminal"}
            </Button>
          </form>
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-muted-foreground">
              ...
              <Link to="/signup" className="text-[#22d3ee] font-semibold hover:underline underline-offset-4 transition-colors">
                Initialize Profile
              </Link>
            </p>
          </div>
